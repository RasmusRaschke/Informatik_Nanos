...
def cal():
    ...
    if on:
        ...
        eigen_correct, x, v, counter = calculate_eigenvalues(v_0, x_0, u_0, u_1, L, N, x_min, x_max, wells, e_min,
                                                             e_max, accuracy_cheap, max_bound, start)
        plot_eigenstates(x, v, eigen_correct, u_0, u_1, counter, x_min, x_max, 2)
        plot_bands(v_0, x_0, u_0, u_1, L, N, x_min, x_max, e_min, e_max, accuracy_cheap, max_bound, start, max_wells)
        return
...
def adv():
    ...
    global on, accuracy_exp, u_0, u_1, max_bound, N, start, x_min, x_max, E, charge, el_field 
    ...
    if on:
        ...
        label_E = tk.Label(master=frame_E, text="Energy of the K-List in [eV]", width=40)
...
label_emin = tk.Label(master=frame_emin, text="emin", width=40)
...
label_emax = tk.Label(master=frame_emax, text="emax", width=40)
...
label_maxwells = tk.Label(master=frame_maxwells, text="max wells", width=40)
...
c = tk.Checkbutton(master = frame_knöpfe, text = "ELectric Field", variable = electric, onvalue = 1, offvalue = 0)
